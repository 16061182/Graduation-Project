...
from utils.fileio import read_data_files, split_data
...
def parse_args():
    ...
    parser.add_argument('--train-test-split', dest = 'train_test_split', default = 0.80, type = float,
    ...
    parser.add_argument('--break-training-at', dest = 'break_training_at', default = 0.99, type = float,
...
def pose_classify_net(X, n_classes):
    with tf.variable_scope('Layer1'):
        weight = tf.get_variable('weight1', initializer = tf.truncated_normal((N_KEYPOINTS, HIDDEN), mean = 0.0, stddev = 0.1))
        bias = tf.get_variable('bias1', initializer = tf.ones((HIDDEN)))
        fc1 = fc_layer(X, weight, bias)

    with tf.variable_scope('Layer2'):
        weight = tf.get_variable('weight2', initializer = tf.truncated_normal((HIDDEN, n_classes), mean = 0.0, stddev = 0.1))
...
if __name__ == '__main__':
    ...
    print('X_data shape: {}'.format(np.array(X_data).shape))
    print('y_data shape: {}'.format(np.array(y_data).shape))
    train_data, test_data = split_data(X_data, y_data, args.train_test_split)
    print('X_train shape: {}'.format(np.array(train_data[0]).shape))
    print('X_test shape: {}'.format(np.array(test_data[0]).shape))
    print('y_train shape: {}'.format(np.array(train_data[1]).shape))
    print('y_test shape: {}'.format(np.array(test_data[1]).shape))
...
# run python command:
# python train_nn.py "./result/match.csv, ./result/fu.csv"